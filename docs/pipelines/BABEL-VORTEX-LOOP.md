...
- v1.62（2026-09-27 凌晨第九輪）：**委派回報是張冠李戴的探針，而閘門看不到這一族**。第九波二十對缺稿
  ...
  林百里的英文名、張懸寫成陳綺貞或唐鳳、夏曼・藍波安寫成蕭美琴，還有一篇我自己驗收放行的 de 新譯。name-consistency
  的規則 B 刻意只比人物頁標題，正文是它的死角。新增 `name-absence-check.py`：譯文用了某人的拼寫、而他的漢字不在
  zh 原文 → 候選。只當報告：同人異稱（蔣中正／蔣介石）、同音異人、譯者補語境三族誤報要人核，存量 146 處進
  OBSERVER-QUEUE #81。**規則**：驗收一篇新譯時，把它提到的每個有頁面的人對一次 zh——agent 的「name-consistency
  0 命中」只代表標題沒認錯人。另兩條同晚修掉：派工單誤判清單改寫後，agent 會自己把「(原文, 說明)」拆開（第九波
  兩隻主動拆）；維護班通報的幣別閘門與雜湊語意見 v1.61 (c)。
...
- v1.61（2026-09-26 晚間第七輪）：**修一篇的殘渣時，追到一支每篇譯文都會經過的修復器在補中文**。
  ...
  超過 50 篇進 OBSERVER-QUEUE #80。**規則**：把一支 fixer 接進譯文熱路徑之前，先問它寫進檔案的
  字是哪個語言的；cjk-leak-check 豁免腳註行，所以腳註是全套閘門的盲區，任何會寫腳註的工具
  都要自帶語言意識。
  (c) 同晚維護班 session 通報兩條，核過屬實：structured-translate 的 `sourceContentHash` 對整份 zh 檔（含 frontmatter）
  取雜湊、又不寫 `sourceBodyHash`，跟 status.py 語意不同（全庫 1,092 篇同 commit 雜湊不符），改用 status.py 的函式；
  幣別閘門只接在委派層，dispatcher 36 小時 444 篇有 133 篇帶裸 yuan／rupiah，`verify_one` 補上（下次重啟生效）。
  **兩條同一個形狀：規則寫進了一條路徑，另一條產線照舊。**接一道閘或一個算法時，grep 所有產出譯文的路徑。
