...
>
> v5.0 設計理由：[reports/rewrite-pipeline-v5-stage-spine-design-2026-05-11.md](../../reports/rewrite-pipeline-v5-stage-spine-design-2026-05-11.md)。修補 v4.1 的兩個結構問題：(1) `# Stage` H1 跟頂部 `# REWRITE-PIPELINE` 同階層平掉；(2) 5 套 `## Step A-X` 並排 grep collision。v5.0 改為 H1 文件 title / H2 Stage / H3 Step N.M / H4 sub-step，agent 線性 read 永遠知道「我在 Stage X, Step X.Y」。
...
**🔴 兩條反射特別強化**（v3.1 sad-shockley 升級，v4/v5 保留）：
...
2. **媒體素材 self-check 🎬** — 不論 Fresh / EVOLVE，Stage 1 Step 1.14 都要跑：
...
1. **Step 1.9 核心矛盾必填** — 找不到矛盾 = 這篇不該被重寫（國防現代化重寫教訓）
2. **Step 1.12 研究報告落檔** — depth-article 沒檔 = 沒 audit trail（DNA #22 raw 永留）
3. **Step 2.4 小標題不編年體** — 編年體 = 維基百科化 = 失敗（Cicada / 草東 / 康士坦 教訓）
4. **Step 4.3.3 aspect ratio 護欄** — portrait hero 切到頭（林琪兒 ι session 教訓）
5. **Step 2.7.5 Agent claim 驗證** — agent 推導的名人 claim 必須有公開 URL Ctrl-F 可搜（吳哲宇 EVOLVE 教訓）
...
| 檔案                                                             | 範圍                                                         |
| ---------------------------------------------------------------- | ------------------------------------------------------------ |
| **本檔**                                                         | 5 stage 線性主流程（單檔，含模式判定 + 媒體生命週期 + cron） |
...
**第一動作**：判定本次 REWRITE 走 4 模式中哪一種。所有模式都進入同一條 Stage 1-5 pipeline，差別只在 Stage 1 Step 1.2 取材方式 + Stage 5 Step 5.4 是否觸發路徑改寫。
...
| ----------------------------------- | -------------------- | ------------------------------------------------------------------ | ----------------------------------- |
| 文章不存在                          | **Fresh**            | 跳 Step 1.2，直接 Step 1.5                                         | 同基本流程                          |
| 文章已存在，需要品質提升            | **Evolution**        | Step 1.2 萃取既有素材 + 標 [LIST-DUMP] / [STUB-TITLE] / [NO-MEDIA] | 同基本流程                          |
| issue 指 N 篇主題重疊可融合進 1 篇  | **Merge variant**    | Step 1.2 多萃 [MERGE-IN] + Step 1.3 選 canonical                   | + Step 5.4 路徑改寫 5 lang redirect |
| issue 指 N 篇主題重疊應分段不減篇數 | **Boundary variant** | Step 1.2 三類劃分 [保留/吸納/移除] + Step 1.4 範圍切片表           | + sibling 反向回補                  |
...
**正確做法**：從舊文中**只提取事實**（Step 1.2），然後從 Step 1.5 開始用全新模式重寫。

> 💡 **實際上 Evolution 模式 = Fresh 模式 + 免費的 Step 1.2 素材**。寫作品質完全相同，只是省了部分研究時間。
...
- frontmatter `image:` + `imageCredit` + §圖片來源 是否齊全？無 → 標 `[NO-MEDIA]`，走 Step 1.14 補跑

#### Merge variant 萃取兩篇的事實

- canonical 的事實清單：照常標 [LIST-DUMP] / [THIN] / 等
- 將被刪那篇的事實清單：標 `[MERGE-IN]`，列出「對方有但 canonical 沒有的視角/場景/數據」
- Step 1.5 之後的研究範圍 = canonical 缺口 + `[MERGE-IN]` 視角的補強查證
...
### Step 1.3: 選 canonical（Merge variant only）
...
### Step 1.4: 範圍切片表（Boundary variant only）
...
#### Step 1.14.2: 圖片素材（hero + inline 圖）+ 授權矩陣
...
#### Step 1.14.4: 媒體授權矩陣三表（research 檔強制）
...
#### Step 1.14.5: Stage 1 收尾 checklist

Stage 1 結束時 deliverable：

- [x] 核心矛盾欄位必填（Step 1.9）— 填不出來 → 不進 Stage 2
- [x] depth-article 研究報告必存（Step 1.12）— `reports/research/YYYY-MM/{slug}.md` 不存在 → 不進 Stage 2
...
- [x] 私有 SSOT 整合過 Step 1.11 觀察者拍板（如有觸發）
...
| --------------------------------------- | ------------------------------------------------------------------------------------------------ |
| 三項全有                                | 已合規，跳過                                                                                     |
| 三項全無（pre-gate 遺珠）               | 補跑 Stage 1 Step 1.14 至少 hero 1 張，append §圖片來源 section                                  |
| Hero 有但 EVOLVE 加的新節主題缺對應視覺 | 評估是否需補 inline 圖（per Stage 4 Step 4.3.1 三段敘事節奏），找不到 PD/CC 圖記錄邊界（不放空） |

**為什麼必須**：

- Stage 1 Step 1.14 的 hard gate 是 2026-04-28 才升，更早 ship 的 article 多為 pre-gate 遺珠
- focused EVOLVE 加新節時容易忽略「既有 article 的媒體狀態」— 假設「上次 ship 已合規」，但 pre-gate 條目實際無 hero
- 找不到合適 PD/CC 圖時不可放空 → 走 fair use editorial commentary scope（per Step 1.14.2 第 8 點）或記錄 search 邊界
...
**依賴**：Stage 1 Step 1.14 必須完成（媒體授權矩陣三表 append research 檔 + 圖片已 cache）。沒做 → 退回 Stage 1 Step 1.14。
...
依 Stage 1 Step 1.14.2 的 manifest 已 cache 完成。Step 4.3.2 僅做最後 verify：
...
- **觀察者直接丟連結**（如林琪兒 ι session）：走 Step 4.3.2-4.3.6 補圖 SOP，不走 Stage 1 Step 1.14
...
| 圖種                  | 路徑                           | 用途                    | 生成方式                                  |
| --------------------- | ------------------------------ | ----------------------- | ----------------------------------------- |
| article hero / inline | `public/article-images/{cat}/` | 文章內容                | Stage 1 Step 1.14 + Stage 4 Step 4.3 手動 |
...
9. **EVOLVE 容易漏 Stage 1 Step 1.14 媒體素材**：pre-2026-04-28 條目多無 hero / 無 §圖片來源 = pre-gate 遺珠，補 EVOLVE 時必查
