...
│   │  Step L. 媒體素材研究（inline 外連 + 圖片 + transcript + 三表）      │
│   │  ─────────                                                           │
...
│   │  Step E. 寫正文 + footnote                                           │
│   │  Step F. 延伸閱讀                                                    │
│   │  Step G. 7 條自檢（歐化 / 60% 暫停數—— / 編年 / 密度 / Agent claim │
│   │           / Title+desc spine sync / 媒體素材 spine check）           │
│   │  Step H. 富文本 + footnote 密度                                      │
│   │  ─────────                                                           │
│   │  Hard gate ↳ 10 條（結尾不罐頭 / 真人引語 ≥ 2 / 富文本達標 /        │
│   │              純中文 / 7 自檢全跑 / 小標題不像第一章第二章）          │
│   ↓                                                                      │
...
│   │     └ frontmatter / format-structure / wikilink / link / cjk-punct  │
│   │       / chronicle-lead / word-count                                  │
│   │  Step B. 多語 visual smoke（i18n 改動時）                            │
│   │  Step C. 媒體插入                                                    │
│   │     ├ 子點 1. 三段敘事節奏（hero / scene-mid / closure）             │
│   │     ├ 子點 2. 圖檔 fetch + cache + naming + EXIF clean              │
│   │     ├ 子點 3. Aspect ratio 護欄                                      │
│   │     ├ 子點 4. Markdown 插入 + caption + alt text                     │
│   │     ├ 子點 5. 授權清單同步（frontmatter + §圖片來源）                │
│   │     └ 子點 6. image-health plugin gate                               │
│   │  ─────────                                                           │
...
**🔴 兩條反射特別強化（v3.1 sad-shockley，v4 保留）**：
...
2. **媒體素材 self-check** — 不論 Fresh / EVOLVE，Stage 1 Step L 都要跑：
...
1. **Stage 1 Step G 核心矛盾必填** — 找不到矛盾 = 這篇不該被重寫（國防現代化重寫教訓）
2. **Stage 1 Step J 研究報告落檔** — depth-article 沒檔 = 沒 audit trail（DNA #22 raw 永留）
3. **Stage 2 Step D 小標題不編年體** — 編年體 = 維基百科化 = 失敗（Cicada / 草東 / 康士坦 教訓）
4. **Stage 4 Step C 子點 3 aspect ratio 護欄** — portrait hero 切到頭（林琪兒 ι session 教訓）
5. **Stage 2 Step G 自檢 5 Agent claim 驗證** — agent 推導的名人 claim 必須有公開 URL Ctrl-F 可搜（吳哲宇 EVOLVE 教訓）
...
- `cat docs/editorial/RESEARCH.md`（方法論：搜尋策略、來源判斷、避坑指南）
- `cat docs/editorial/RESEARCH-TEMPLATE.md`（填空模板）

## Step A：模式識別

**第一動作**：判定本次 REWRITE 走 4 模式中哪一種。所有模式都進入同一條 Stage 1-6 pipeline，差別只在 Stage 1 Step B 取材方式 + Stage 5 Step D 是否觸發路徑改寫。
...
| 場景                                | 模式                 | Stage 1 差異                                                     | Stage 5 差異                      |
| ----------------------------------- | -------------------- | ---------------------------------------------------------------- | --------------------------------- |
| 文章不存在                          | **Fresh**            | 跳 Step B，直接 Step C                                           | 同基本流程                        |
| 文章已存在，需要品質提升            | **Evolution**        | Step B 萃取既有素材 + 標 [LIST-DUMP] / [STUB-TITLE] / [NO-MEDIA] | 同基本流程                        |
| issue 指 N 篇主題重疊可融合進 1 篇  | **Merge variant**    | Step B 多萃 [MERGE-IN] + 選 canonical                            | + Step D 路徑改寫 5 lang redirect |
| issue 指 N 篇主題重疊應分段不減篇數 | **Boundary variant** | Step B 三類劃分 [保留/吸納/移除] + 跨 phase handoff              | + sibling 反向回補                |
...
**正確做法**：從舊文中**只提取事實**（Step B），然後從 Step C 開始用全新模式重寫。

> 💡 **實際上 Evolution 模式 = Fresh 模式 + 免費的 Step B 素材**。寫作品質完全相同，只是省了部分研究時間。
...
- frontmatter `image:` + `imageCredit` + §圖片來源 是否齊全？無 → 標 `[NO-MEDIA]`，走 Step L 補跑

### Merge variant 額外動作

**選 canonical（在 Step B 之前）**：比較候選文章，挑一篇當保留方。判準（按優先序）：
...
- Step C 之後的研究範圍 = canonical 缺口 + `[MERGE-IN]` 視角的補強查證
...
### Boundary variant 額外動作

**範圍切片表（在 Step B 之前）**：對所有涉及篇章做一次 audit，產出範圍切片表：
...
⚠️ **萃取完畢後，舊文不再被參考。只看事實清單進入 Step C。**
...
研究時就標出 2-3 個候選結尾畫面，Stage 2 Step B（結尾先行）直接挑用。
...
如果發現高度重疊的既有文章 → 改走 Evolution / Merge / Boundary 模式（回 Step A 重判）。
...
## Step J：研究報告必存 `reports/research/YYYY-MM/` 📁
...
### 子點 1. inline 外連 manifest（YouTube／影像／音檔）
...
### 子點 5. Stage 1 收尾 checklist

Stage 1 結束時 deliverable：

- [x] 核心矛盾欄位必填（Step G）— 填不出來 → 不進 Stage 2
- [x] depth-article 研究報告必存（Step J）— `reports/research/YYYY-MM/{slug}.md` 不存在 → 不進 Stage 2
...
- [x] 私有 SSOT 整合過 Step I 觀察者拍板（如有觸發）
...
### 自檢 6：Title + description spine sync 🔴
...
| --------------------------------------- | --------------------------------------------------------------------------------------------------- |
| 三項全有                                | 已合規，跳過                                                                                        |
| 三項全無（pre-gate 遺珠）               | 補跑 Stage 1 Step L 至少 hero 1 張，append §圖片來源 section                                        |
| Hero 有但 EVOLVE 加的新節主題缺對應視覺 | 評估是否需補 inline 圖（per Stage 4 Step C 子點 1 三段敘事節奏），找不到 PD/CC 圖記錄邊界（不放空） |

**為什麼必須**：

- Stage 1 Step L 的 hard gate 是 2026-04-28 才升，更早 ship 的 article 多為 pre-gate 遺珠
- focused EVOLVE 加新節時容易忽略「既有 article 的媒體狀態」— 假設「上次 ship 已合規」，但 pre-gate 條目實際無 hero
- 找不到合適 PD/CC 圖時不可放空 → 走 fair use editorial commentary scope（per Stage 1 Step L 子點 2 第 8 點）或記錄 search 邊界
...
承襲 Stage 2 Step G 自檢 6（已在 Stage 2 跑過）。Stage 3 再 grep 一次確認：
...
**⚠️ 格式不合格 = 修正後重新檢查。不進 Step C。**

## Step B：多語 visual smoke test（i18n 改動時）
...
**觸發時機**：Step A format-check 通過後、Stage 5 cross-link 之前。

**為什麼這時插入**：寫完 prose 才知道「實際敘事節奏在哪、哪段需要 visual 呼吸」。寫之前布陣會綁死寫作節奏；寫完一次插入更自然。

**依賴**：Stage 1 Step L 必須完成（媒體授權矩陣三表 append research 檔 + 圖片已 cache）。沒做 → 退回 Stage 1 Step L。
...
### 子點 2. 圖檔 fetch + cache + naming

依 Stage 1 Step L 子點 2 的 manifest 已 cache 完成。Step C 子點 2 僅做最後 verify：
...
### Stage 4 Step C 邊界與例外

- **Hub 頁**（`_*.md`）：不放圖，跳過 Step C
- **短修正 / heal commit**：不重新走 pipeline，圖用既有的不動
- **翻譯文**：跟原文圖同步（cache 共用），caption 翻譯成對應語言
- **沒有合適媒體素材**：明確標 `no-media` 進 research 檔，跳過 Step C
- **觀察者直接丟連結**（如林琪兒 ι session）：走子點 2-6 補圖 SOP，不走 Stage 1 Step L
- **Article ship 後才發現缺圖**：spawn `heal:` commit + 走 Step C

### 跟 spore 配圖區分

| 圖種                  | 路徑                           | 用途                    | 生成方式                             |
| --------------------- | ------------------------------ | ----------------------- | ------------------------------------ |
| article hero / inline | `public/article-images/{cat}/` | 文章內容                | Stage 1 Step L + Stage 4 Step C 手動 |
...
## Step D（Merge variant only）：Astro redirect 5 lang + 刪舊檔

整併獨有的收尾，**四件事缺一不可**：

### 子點 1. Astro redirect（5 lang 全寫）
...
每篇單獨走完整 Stage 1-5 流程。Step B 雙向延伸閱讀時要互相反向回補（C 寫完 → 加進 B/D 延伸閱讀；B 寫完 → 加進 C/D；以此類推），形成完整 sibling 網路。
...
git commit -m "rewrite: [文章名] — EDITORIAL v6.3 + Pipeline v4.0"
git push
```
...
REWRITE 是 routine 飛輪 6 條核心 routine 之一（`twmd-rewrite-daily`）。每天 16:16 自動跑：
...
7. **結尾最後寫 = 品質最差**：v2 改成結尾先行（Stage 2 Step B）
8. **觀察者反覆 callout 同問題 → DNA #15 反覆浮現要儀器化** → 升 plugin gate（chronicle-lead / word-count / Title+desc spine sync）
9. **EVOLVE 容易漏 Stage 1 Step L 媒體素材**：pre-2026-04-28 條目多無 hero / 無 §圖片來源 = pre-gate 遺珠，補 EVOLVE 時必查
...
git commit -m "🧬 [semiont] rewrite: {文章名} — EDITORIAL v6.3 + Pipeline v4.0"
git push
```
...
- **v2.20**（2026-04-28）— 新增 Stage 1.7 媒體素材研究 + Stage 4.5 媒體插入（v4 收進 Stage 1 Step L + Stage 4 Step C）
