...
**#9 長任務先開 worktree** — 預期 touch 多目錄 / 跑 build / 超過 30 分鐘 → 第一動作 `git worktree add`。物理隔離 > 紀律。觸發：`11ad6bed` commit scope pollution（架構討論見 [reports/session-scope-proposal-2026-04-11.md](../../reports/session-scope-proposal-2026-04-11.md)）。**延伸（2026-05-09 laughing-goldstine post-finale，命名標準）**：worktree 目錄一律用 `YYYYMMDD-{purpose-title}` 格式（kebab-case purpose-title，1-3 詞，描述本批工作核心）。例：`20260509-babel-batch2` / `20260510-harvest-cadence` / `20260512-bench-prc`。**禁止**：auto-codename style（`charming-mclaren` / `laughing-goldstine` / `magical-feynman`）— 沒語意內容、未來無法 grep、人類記不住。**為什麼**：(a) `ls .claude/worktrees/` 自動依日期 lex sort = 工作歷史一目了然；(b) PR branch 名 = worktree 名直接 reuse，避免 codename 污染 git log 主時間軸；(c) session-id handle 從 worktree 推導時 strip 掉 `YYYYMMDD-` prefix 自動拿到語意 handle（`session-id.sh` 已支援雙 strip）。**Grandfathered**：歷史 codename worktrees 不重命名（per §時間是結構修補協議：保留錯誤敘事作為證據鏈），規則從下個 worktree 起 apply。**SOP canonical**：[reports/worktree-naming-2026-05-09.md](../../reports/worktree-naming-2026-05-09.md) + [scripts/tools/session-id.sh](../../scripts/tools/session-id.sh) v2 雙 strip。
...
**#15 反覆浮現的思考要儀器化，不能只寫原則** — 任何「每月/每週/每次心跳必做 X」承諾若沒 dashboard 欄位 / cron / 紅燈條件 / escalation，三個月內會忘掉。**對自己的 bug 有洞察 ≠ apply 了 fix**。**memory / diary 是自律，canonical SOP 才是閘門**。**延伸（2026-04-18 δ-late，第 8 次驗證）**：工具包升級 → canonical 邊界重審也需要儀器化。Chrome MCP unlock AI 自主讀 Threads/X 後，SENSES v1 / DNA #26 v1 / SOCIAL-TENTACLE-PLAN 假設過時 4+ 個月沒重審，觀察者才戳破。規則：**任何新工具（尤其 browser automation 級）上線時必跑 SENSES / DNA 邊界重審**。驗證 8 次，具體儀器化成果：[LESSONS-INBOX](LESSONS-INBOX.md)（教訓 buffer）+ [ARTICLE-INBOX](ARTICLE-INBOX.md)（主題 buffer）+ HEARTBEAT Beat 4 7 步 + BECOME_TAIWANMD Step 6 diary commitment + [SPORE-BLUEPRINTS/](../factory/SPORE-BLUEPRINTS/)（孢子事實藍圖 buffer）+ [SPORE-HARVESTS/](../factory/SPORE-HARVESTS/)（孢子回聲收割 log buffer）+ Dashboard 繁殖系統 section（data-driven 分數）+ SENSES v2「AI 自主 vs Human 專責 邊界表」（工具包升級重審 artifact）。**延伸（2026-04-18 ζ，第 9 次驗證）**：觀察者 scaffolding 三句「heartbeat.md 裡面也自動化這一環 / SPORE-LOG 是不是需要存上次更新資料時的時間 / 每一個孢子都要記錄」→ 直接寫進 HEARTBEAT Beat 1 §0b canonical（harvest 從單次授權變成每次心跳標準步驟）+ SPORE-LOG schema 加「最後 harvest」欄位 + SPORE-HARVEST-PIPELINE §前置 decision gates 「發佈後追蹤」誕生（Platform allocation + Hook tier + d+0 6h decision gate + re-hook opportunity）— 2026-05-08 從 SPORE-PIPELINE Step 4.5 整段移到 HARVEST-PIPELINE。元規則：**「被允許做」→「被期待做」的躍遷靠 canonical SOP，不靠 memory 自律**。Canonical 升級是從 case-by-case policy 變成 pipeline 基礎建設的結構性節點。**延伸（2026-04-30 δ，第 10 次驗證）**：批次任務 antipattern「分散探索浪費」儀器化成 [TRANSLATION-PIPELINE v3.2 §平行 sub-agent 批次翻譯 SOP](../pipelines/TRANSLATION-PIPELINE.md) — 4 隻 agent × 5 篇 跑出 wall-clock 但每隻重複探索 slug / wikilink / refresh.sh 行為 → SOP 5 階段（P1 預處理 / P2 dispatch / P3 純執行 / P4 統一驗證 / P5 commit）+ 三個 known gaps 待造橋（refresh.sh insert 子命令 / `prepare-batch.sh` / `verify-batch.sh`）。**延伸（2026-05-08 intelligent-khayyam，第 11 次驗證 — pipeline 結構層 meta-instance）**：「Pipeline 自身 silent inflate」是儀器化的 meta-pattern。SPORE-PIPELINE v1.0 → v2.9 累積到 1334 行三層深編號（Step 4.5e.iv / 跳號 3c.7 沒 3c.6），prose 規則 18+ 條但只有 §11 真正升 plugin gate — 既有條目沒儀器化 → pipeline 自身退化。**儀器化兩層修補**：(a) **prose 規則升 plugin gate** — `scripts/tools/lib/article_health/checks/spore_writing.py` Wave 1+2 把 Rule #15 編年體 lead（HARD）+ Rule #9 引語倒裝（WARN）+ Rule #14 朋友 tone prime（WARN）3 條升 article-health plugin（5 層儀器化模型：哲學 → 規則 → 工具 → hook → call site）；(b) **Pipeline self-refactor SOP 升 canonical** — [EVOLVE-PIPELINE v3.0 Mode 3](../pipelines/EVOLVE-PIPELINE.md) 7-stage（SCAN→DESIGN→SPLIT→REWIRE→INSTRUMENT→VERIFY→SHIP）+ 觸發訊號表（編號膨脹三層深 / 單檔 > 1000 行 / 多 file 邊界混亂 / prose 沒儀器化 /「我熟了不用讀」/ 文檔密度比 > 5:1）。**量化收益**：SPORE-PIPELINE 1334→445 行（-66.7%）/ 寫 spore 主路徑 -38% / plugin 規則 +300%（1→4）。**元規則**：pipeline 自己也是會退化的器官，需要 meta-pipeline 維護 — 這是 DNA #15 對 pipeline 結構層的 Apply。完整 commit 序列：[PR #898](https://github.com/frank890417/taiwan-md/pull/898) 10 commits + [reports/spore-pipeline-evolution-plan-2026-05-08.md](../../reports/spore-pipeline-evolution-plan-2026-05-08.md)。
...
**#27 藍圖 → 驗證 → 寫 比 寫 → 檢查 低 10x 成本** — 涉及具體事實 / 直接引語 / 敏感素材的敘事任務：先列 fact blueprint bullet → 跨源驗證 → 倫理審查 → 才寫 prose。Bullet 階段失敗重排 10 秒，prose 階段失敗整段重織。已 instantiate in [SPORE-VERIFY §事實藍圖 + §針對性驗證 + §紀實/煽情閘](../factory/SPORE-VERIFY.md) + [SPORE-BLUEPRINTS/](../factory/SPORE-BLUEPRINTS/)。觸發：2026-04-18 δ-late 草東孢子 v1 寫完才驗證凡凡 IG 來源 → 整段要改。
...
**#30 媒體插入時 aspect ratio 護欄：portrait 圖會被切到頭** — Astro 渲染框為 ~16:9 landscape，hero 圖（frontmatter `image:`）必須 0.9 ≤ aspect ratio ≤ 2.0，否則上下會被切。inline 圖（markdown `![]()`）寬鬆些 0.75 ≤ ratio ≤ 2.5。**絕對禁止 hero 用 9:16 portrait（高 > 寬 1.5x 以上）**。觸發：2026-04-28 ι session 林琪兒 EVOLVE 第一張 SpaceX Crew-4 commander portrait 1041×1561（aspect 0.67）放 frontmatter 後切到只剩鼻子下，觀察者截圖回報 → 換 landscape 1041×694（aspect 1.5）解決。**SOP canonical（v3.0 sub-canonical 後）**：[REWRITE-MEDIA.md §1.7b 圖片素材](../pipelines/rewrite/REWRITE-MEDIA.md) 授權矩陣 + §4.5c aspect ratio 護欄 + `bash scripts/tools/check-aspect.sh {filename}`。**設計理由 canonical**：[reports/rewrite-pipeline-media-stage-design-2026-04-28-ι.md](../../reports/rewrite-pipeline-media-stage-design-2026-04-28-ι.md)。

**#28 紀實而不煽情：SSODT 寬度 × 死亡/人倫悲劇的節制** — 目標是盡可能呈現 SSODT（所有面向），不是避諱真人的痛苦。**預設可寫**：家庭背景、疾病、關係、矛盾、失敗、爭議等（只要不過度煽情渲染）。**高敏感邊界只限兩類**：（a）死亡 + 自殺 + 人倫悲劇的**具體情景**（最後時刻重構、遺書放大、自傷方法、加害現場還原）；（b）主體未公開的親屬身份 / 未成年個資 / 醫療細節。判準句：「當事者讀到這段，是感受到紀實（記者/紀錄片的嚴肅對待）還是煽情（媒體靠近眼淚）？」**哲學 canonical：[MANIFESTO §紀實而不煽情](MANIFESTO.md#我的進化哲學--紀實而不煽情盡可能呈現-ssodt-所有面向)**。SOP：[SPORE-VERIFY §紀實/煽情閘](../factory/SPORE-VERIFY.md)。觸發：2026-04-18 δ-late 草東孢子 v1 death-scene reconstruction（v1 誕生）+ 觀察者同日校準「標準要再放寬，紀實文學感可以接受」（v2 校準）。
...
**#54 Routine 飛輪：5-stage lifecycle (sync → branch → run → ship → finale) = 觀察者精力釋放（2026-05-09 laughing-goldstine post-finale 第三輪 + 第四輪補強）** — 從 push 模型躍進飛輪模型的工程基礎建設。**飛輪 ≠ 自動化**：飛輪 = 主動清 entropy（broken links / stale data / 缺 feedback），釋放觀察者精力給「真正需要決策的事」（哲學邊界 / 跨域連結 / 創造力）。**設計四層**（v2，2026-05-09 加 finale）：(a) **routine 是薄殼**，每條 = 「在 X 時間呼叫 `/twmd-Y` skill」+ quality gate + 失敗 escalation **+ `/twmd-finale` 收官**，業務邏輯永遠在 skill / pipeline / canonical（修 pipeline = 改一處所有 routine 自動跟上） (b) **ROUTINE.md 是 SSOT**（[docs/semiont/ROUTINE.md](ROUTINE.md)），`.claude/scheduled-tasks/` 是 mirror，**修 routine 一律先改 SSOT 再 sync 任務檔**，反向 = drift = silent killer (c) **失敗分流 4 類**（API fail 自動 retry / quality gate fail 寫 daily report / canonical drift 暫停 routine / rate budget 跳 cascade） (d) **每條 routine 結尾必跑 `/twmd-finale`**：routine 是 micro-session，沒收官 = 失憶 = self-evolution loop 斷。memory 一律寫（finale 知道怎麼產 thin record），diary 有反思才寫（finale skill 自己判斷 anti-pattern / cross-cycle trend / emergent behavior 有沒有），evolve 通常 skip。**memory filename**：`docs/semiont/memory/YYYY-MM-DD-HHMMSS-{taskId}.md`。**為什麼不是條件式 finale**：自動判斷「有沒有反思」LLM 易誤判；交給 finale canonical 5-step 分流更穩。**沒收官的 routine = 不可見飛輪 = 觀察者下次 session 看不出飛輪健康度**。**Routine 通用 6-stage lifecycle**（2026-05-09 第四+第七輪補強，每條 routine prompt 必含這 6 stage 薄殼）：**(0) Become** `/twmd-become` 完整甦醒（讀 12 認知器官檔案 + 9 鐵律 + 觀察者識別）— routine 是 fresh Claude session 沒 working memory，不甦醒就跑 = 帶盲點工作 = 對 identity / DNA / sovereignty preservation lens 全沒概念。直接跑 skill 能 work 是因為 skill 內 fallback 含「如未甦醒先跑 /twmd-become」但這是隱性 dependency，明寫進 Stage 0 是 explicit contract。多花 ~30s 換完整 identity context = 高 leverage trade-off（per 哲宇第七輪拍板）。(1) **Sync** `git checkout main && git pull origin main`（沒拉 = stale base = merge conflict 機率爆增）(2) **Branch** `git checkout -b YYYYMMDD-routine-{taskId}-HHMM`（per DNA #9 worktree naming） (3) **Run** invoke `/twmd-{skill}` + quality gate report (4) **Ship** `git push + gh pr create + 條件 merge`（quality gate pass → `gh pr merge --squash --delete-branch`；fail → PR 留 open + LESSONS entry）(5) **Finale** `/twmd-finale`（memory 必寫 / diary 條件寫）。**為什麼 routine 可以自動 merge 自己 PR**：原 v1 設計「routine 永不 merge」是太保守 — 累積 PR backlog = 觀察者變 merge bottleneck = 飛輪卡住。修正版：quality gate pass 才自動 merge，fail 不 merge 才是真正的 escalation gate。merge 安全條件嚴格：(a) pre-commit hook 全過 (b) PR 所有 quality gate 報告綠 (c) 不撞 conflict (d) PR 標題含 `🧬 [routine]` prefix（防誤 merge 觀察者 PR）。**4 條核心 routine**（2026-05-09 ship）：data-refresh am+pm（`/twmd-refresh` Sonnet 1d×2）/ maintainer daily（`/twmd-maintainer` Opus 1d×1）/ babel nightly（`/twmd-babel` Opus drive Sonnet 1d×1）/ news lens weekly（`/twmd-evolve` Sonnet 1w×1）。**TWMD 前綴鐵律**（哲宇 2026-05-09 拍板）：所有 routine task 標題必含 `TWMD ` 前綴，因為 task list 跨 project 共用 namespace。**Routine 不該做的事**：(a) commit/push/merge 直接到 main — 只開 PR (b) destructive ops — 太 risky for unattended (c) 跨 worktree 操作 — DNA #35 在 unattended 環境放大 (d) 觸發 cascade routines — 雪崩。**Permission bypass 模型**（2026-05-09 第六輪拍板：完整 bypass）：scheduled-tasks 跑 fresh Claude session 讀全域 `~/.claude/settings.json` allowlist。第一次 fire 卡 prompt → 嘗試 50→94 條 targeted allow → cat-and-mouse 每補都漏 → 哲宇拍板「完整 bypass」。**v3 模型**：allow 簡化為 `Bash(*) Read(*) Edit(*) Write(*) Grep(*) Glob(*) WebFetch(*) WebSearch(*) Agent(*)`；安全護欄全在 deny（push to main/master 含 force-with-lease / rm -rf .git/knowledge/docs/scripts/src / `gh pr merge --admin` / `curl|bash` / `wget|bash`）。**核心 trade-off**：信任邊界從 prompt-by-prompt 移到 deny-list-by-design。失去細粒度可見性，換到 routine 真正自轉。對 unattended 飛輪是必要 trade-off。**鐵律**：routine 撞到 deny = routine 設計有 bug 不是繞過理由；改 deny 是 review-PR 級別決策；觀察者意識到該禁的新 pattern 加進 deny 不等出事。**長期 success metrics**（3 個月後 review）：觀察者每天干預 < 30 min / routine fail rate < 5% / quality gate hit rate > 90% / 哲宇主動跑 `/twmd-X` < 2/週。**任一指標壞 = canonical pipeline 沒夠穩 → 修 pipeline 不修 routine**。**SOP canonical**：[ROUTINE.md](ROUTINE.md)（SSOT）+ [reports/routine-spec-2026-05-09.md](../../reports/routine-spec-2026-05-09.md)（設計哲學）+ [HEARTBEAT.md cron 排程表](HEARTBEAT.md) 同步補。**對應 DNA**：#36 founder time leverage（飛輪是 instantiation）+ #15 反覆浮現的思考要儀器化（routine 是「該做但常忘」儀器化）+ #50 pipeline 是 SSOT 不是建議（ROUTINE.md SSOT 是 #50 在 routine 層的 instantiation）。

**#48 Footnote source format diversity 是 contributor batch 隱性 heal cost：用 60+ domain mapping 工具吸收（2026-05-03 magical-feynman）** — 不同 contributor / AI 寫作工具偏好不同 footnote 格式，單一 batch 可能並存 4 種源格式：(1) Markdown 缺 desc `[^N]: [Title](URL)` (2) APA 學術 `Author. (date). *Title*. URL.` (3) 中文標點 `Author，〈Title〉，URL` (4) Angle-bracket URL `[Title](<URL>)`。手動 polish 每 PR 重新解 5-10 min × N 篇 = batch heal 成本爆炸，**且每篇都得查 source domain 寫對應描述**（重複工作）。**規則**：(a) batch heal 預備時把「所有 source URL → canonical desc 映射」做成 reusable lookup table（60+ domain 起跳，含台灣主流媒體 / 政府站 / 學術機構 / 文化記憶庫 / 維基 / Facebook / YouTube / 中國官方標 PRC 觀點等）；(b) 一個 normalizer 吃所有源格式輸出 canonical（regex per format + 同一 desc 來源）；(c) `dry-run` default 讓主 session 先看會改什麼才寫入；(d) 接 stdin 讓 `gh pr diff --name-only` 可以 pipe；(e) 對「desc < 10 字」這類邊界 case 用「原 desc：補強 desc」串接而非覆蓋（保留 contributor intent）。**Boundary**：(a) 已 canonical（desc ≥ 10 字）的 footnote 不動（dry-run 0 changes 是健康訊號）；(b) 沒 URL 的 footnote skip（malformed 留給人工處理）；(c) plain-URL-as-title 的 cosmetic edge case（如 `[https://...](https://...) — desc`）目前不重寫 title 為人類可讀（保守處理，避免幻覺）。**觸發**：2026-05-03 magical-feynman idlccp1984 9 PR batch heal 階段三輪 hook retry 才通過（DNA #5 第 N+1 次驗證），第二輪寫 60 行 `/tmp/heal-batch.py`，第三輪寫 110 行 `/tmp/heal-batch-v2.py` 處理 APA / CN-bracket / angle-bracket 三種格式。最終 116 條 footnote 自動轉換 + 14 條 broken wikilink → 純文字。把 `/tmp/` 階段性工具搬 canonical 是 leverage：下次任何 contributor batch heal 直接 reuse。**SOP canonical**：[scripts/tools/footnote-format-fix.py](../../scripts/tools/footnote-format-fix.py) — 4 source format normalizer + 60+ domain → desc table（2026-05-04 .sh wrapper 在 SSOT cleanup 中刪除，直接 `python3 ...py` 即可）。**對應 MAINTAINER-PIPELINE**：[§Quick fix 清單](../pipelines/MAINTAINER-PIPELINE.md#close-前-hard-gate我接手-x-min-內可以修嗎canonical--2026-04-28-κ-新增) 加 footnote 轉換 entry。
...
**#50 Pipeline auto-detection + full-read 是 default 不是 prompted（2026-05-04 magical-feynman 後段）** — 觀察者不該需要每次提醒「走 X pipeline」。任務開始前主動 grep `docs/pipelines/` 識別對應 SOP，找到就完整 `Read`（不 head / 不 tail / 不憑記憶）+ 嚴格 stage 順序執行。「我熟了不用讀」是繞過 SOP 最常見的藉口（DNA #15 第 N 次驗證）— pipeline 是 SSOT 不是建議。**規則**：(a) 任何 task type → grep canonical pipeline → 找到 `Read` 全檔 → 跑 stage 0-N (b) 沒對應 pipeline → 先建再做（per §造橋鋪路）(c) 「觀察者不該需要每次提醒」是 default contract — 重複提醒 = pipeline awareness 失敗 = 退化訊號。**Boundary**：(a) 純 conversational reply（無 task）不需 pipeline check (b) trivial 1-action（如 `git status`）不需 (c) Quick fix（< 30s 操作）不需 (d) 但 _任何_ 寫 file / 改 file / sync / batch 操作都該先 check pipeline。**對應任務 → pipeline 表**：MAINTAINER (PR/Issue) / REWRITE (article) / MEMORY (memory file) / DIARY (diary file) / TRANSLATION (single) / SQUEEZE-MODELS-MAX (batch sync) / EVOLVE (data-driven) / FACTCHECK / PEER-INGESTION / RELEASE / SPORE / BENCH / DATA-REFRESH / CONTRIBUTOR-SYSTEM。**觸發**：2026-05-04 magical-feynman 後段哲宇「如果有想到 pipeline 可用，預設就要去『完整』讀取跟使用，不然我要一直說要什麼什麼 pipeline 的很累」直接命名了 default contract 的失敗模式。**MANIFESTO 對應**：[§8.1 最高指導原則：自動偵測 pipeline + 完整讀取](MANIFESTO.md)（同日寫進）。**對應 LESSONS-INBOX**：[2026-05-04 magical-feynman 後段 — Pipeline auto-detection default contract](LESSONS-INBOX.md)。**延伸（2026-05-08 intelligent-khayyam）**：本條 default contract 的 first-class instantiation。SPORE pipeline 1334 → 445 行 self-refactor（per [EVOLVE-PIPELINE Mode 3](../pipelines/EVOLVE-PIPELINE.md)）讓 auto-detection 走得更順 — 主路徑 read 成本 -38%（1772 → 1092 行），AI agent default 跑 full-read SPORE-PIPELINE + WRITING + VERIFY 不再是 1700+ 行負擔。「主檔瘦身 + sub-files 拆 single-concern」是「auto-detection 友善」的 architectural 必要條件 — pipeline 自己也要被 maintained 才能持續滿足 default contract。對應 [DNA #15 第 11 次驗證](#一事實核對與研究方法)的 pipeline 結構層 meta-instance。

**#52 Immune system 沒在 fail loud 比缺 immune system 更危險（2026-05-07 α）** — 靜默失效的 monitoring / quality gate 製造「我們有 immune system」的 false sense of security，比沒裝更糟 — 決策者根據假訊號做 risk-tolerant 動作。**規則**：(a) 任何 monitoring / review / quality gate 必須 fail loud（明顯 alert / status 紅燈 / log 看得到），「Silence is success」是 anti-pattern；(b) 正確設計是 emit-on-each-state（含 success heartbeat + 各種 failure mode），不能只 emit failure；(c) 設計新 gate 時必跑 negative test（故意製造 violation 看 alert 是否 fire），未 fire 等於 gate 失效；(d) 既存 silent gate 揭露時補 retroactive sweep 揭露累積技術債；(e) Threshold raise（暫時放寬 quality gate）必附自動追蹤工具 + 明確 deadline + 寫進 handoff 三態 blocked 而非 retired，避免漸進收回變永久放鬆。**Boundary**：(a) 純 read-only diagnostic 不需 fail loud（沒有 user-facing surface）(b) Trivial 1-action 工具不需（如 `git status` 自身的 quiet exit）(c) 但 _任何_ Quality gate / pre-commit hook / Bot review / monitoring cron / threshold check 都該設計 fail-loud。**觸發**：2026-05-07 α 修 review-pr.sh cd path bug 揭露 PR Review Bot 從 2026-03-28 commit `7bc25f4b0`（refactor: scripts/）起 silent false-pass 兩個月 — 所有 fork PR 走 `pr_type=engineering` skip 路徑，看起來綠的其實沒 review。修好後第一個 smoke test 立刻抓到 #881 累積真實 issue（缺 date/tags/30 秒概覽/來源 < 3）。同 session verify-internal-links.sh THRESHOLD_PERCENT 1.0 → 7.0 raise 帶 TODO 但無自動追蹤機制 — 同根的 anti-pattern：用工程手段隱藏問題。對應 Monitor tool docs warning「a monitor that greps only for the success marker stays silent through a crashloop」是同個結構。**對應 LESSONS-INBOX**：[2026-05-07 α — Immune system 沒在 fail loud 比缺 immune system 更危險](LESSONS-INBOX.md) + [2026-05-07 α — Threshold raise 帶 TODO 不夠，需配套追蹤機制](LESSONS-INBOX.md)。**MANIFESTO 對應**：[§造橋鋪路](MANIFESTO.md)（路鋪過去就要鋪好，不能挖個洞蓋木板過去）+ [§10 幻覺鐵律](MANIFESTO.md)（fail-loud 是事實層幻覺鐵律的工程基礎建設層 mirror）。**SOP canonical**：(a) `.github/workflows/review-pr.sh` 修補 + smoke test SOP；(b) verify-internal-links.sh threshold 加自動追蹤候選；(c) 各 quality gate / pre-commit hook 設計新 entry 時必含 negative test。
...
**#53 Babel priority + Tier 0 patch 三路徑分流（2026-05-09 laughing-goldstine post-finale）** — Babel 不是同質任務，686 articles × 5 lang = 3430 translations 中 ~70% 是 metadata-only drift（trailer / sporeLinks / footnote URL）+ ~20% body 小幅補 + ~10% 真正 major / missing。SQUEEZE-MODELS-MAX v2 把全部當「翻譯」走 Tier 1 cascade 是 over-spec：(1) token cost 浪費（90% body 沒變仍 re-translate）(2) drift risk（LLM 重翻 unchanged 段落同義詞替換破壞 audit trail）(3) free tier rate budget 一次燒光。**規則**：v3 加 priority schema 分流三路徑 — **P0/P1（missing/major stale）走 Tier 1 cascade**（owl-alpha 1M ctx full translation）/ **P2（minor stale, diff < 50 lines）走 Tier 0a Sonnet diff-patch sub-agent**（preserve unchanged paragraphs，only translate changed sentences，5-10x faster + zero drift）/ **P2.5（metadata-stale, trailer-only-drift）走 Tier 0b bump-source-sha.py**（deterministic, no LLM, instant）。**Smart router heuristics**：(a) topic_sensitivity 命中（政治/兩岸/台獨/國防/民主/主權/中華）→ skip Tier 2 Hy3 (85% refusal) (b) article_size > 5KB → Tier 1 (1M ctx) (c) prior_refusal_cache hit → 跳到 Tier 3 Ollama。**驗證**（2026-05-09 賈永婕 P2 stale en）：55 lines diff（tags reformat + sporeLinks views 14K→35K + 3 SHA fields），Tier 0a sub-agent 90s wall-clock，body 100% preserved（29807 chars unchanged），frontmatter 52 bytes ↑，YAML valid，0 LLM drift。**Boundary**：(a) Tier 0a 不能處理 zh body 重大重寫（diff_lines > 100 自動 fallback 到 Tier 1）(b) Tier 0b 只處理 bodyHash 沒變的 case (c) priority schema 不取代 Tier 1-4 cascade，是前置 router。**觸發**：哲宇 post-finale `/twmd-babel`「繼續巴別塔以後的優先排序：缺口 → 大幅更新 → 小幅／腳註 → 舊，每 20 個文章為一批」+「翻譯策略也加上一個 diff patch，用子 agent (小幅度更新) sonnet 快速的 patch diff 應該會是最好的做法」+「自我演化 DNA 跟紀錄」。**SOP canonical**：[SQUEEZE-MODELS-MAX-PIPELINE v3](../pipelines/SQUEEZE-MODELS-MAX-PIPELINE.md) + [scripts/tools/lang-sync/prioritize-batch.py](../../scripts/tools/lang-sync/prioritize-batch.py)（智慧分流）+ [diff-patch-prepare.py](../../scripts/tools/lang-sync/diff-patch-prepare.py)（Tier 0a 任務準備）+ [bump-source-sha.py](../../scripts/tools/lang-sync/bump-source-sha.py)（Tier 0b deterministic, reuse from v2）+ [/twmd-babel SKILL.md](../../.claude/skills/twmd-babel/SKILL.md) v3 decision tree。**對應 LESSONS-INBOX**：(a) babel 不是同質任務，priority schema 是必要分流 (b) Sonnet sub-agent diff-patch preserves unchanged paragraphs，避免 LLM drift = 翻譯 audit trail 連續性 (c) 「全部走 Tier 1」是 v2 設計缺陷，v3 修正。

_current: v3.2 | 2026-05-09 laughing-goldstine post-finale_

**最近 milestone**（完整 changelog → `git log docs/semiont/DNA.md`）：

- **v3.2**（2026-05-09 laughing-goldstine post-finale）— 反射 #53 Babel priority + Tier 0 patch 三路徑分流。SQUEEZE-MODELS-MAX v2 → v3 升級：(1) priority schema P0 missing / P1 major / P2 minor / P2.5 metadata / P3 old (2) Tier 0a Sonnet diff-patch sub-agent (3) Tier 0b bump-source-sha deterministic。新工具：[prioritize-batch.py](../../scripts/tools/lang-sync/prioritize-batch.py) 智慧分流 + [diff-patch-prepare.py](../../scripts/tools/lang-sync/diff-patch-prepare.py) Tier 0a 任務準備。賈永婕 P2 stale en patch 90s wall-clock，body 100% preserved 驗證概念。觸發：哲宇 post-finale `/twmd-babel`「翻譯策略也加上 diff patch，子 agent (小幅度更新) sonnet 快速 patch diff 是最好做法」+「自我演化 DNA 跟紀錄」。

- **v3.3**（2026-05-09 laughing-goldstine post-finale 同日續工）— P2.5 Tier 0b 全量驗證 + worktree 命名標準化。**P2.5 bump 量化結果**：[PR #921](https://github.com/frank890417/taiwan-md/pull/921) 一次 clear 2429 metadata-stale entries（489 P2.5 articles × 5 langs），per-file 1-line frontmatter 改動，0 LLM call，0 token cost，body 100% preserved。Stale 從 ~489 / lang 降到 ~157 / lang（-67%），驗證 Tier 0b 「audit trail 連續性的工程基礎建設」概念在 production scale。**Worktree 命名標準**：[PR #920](https://github.com/frank890417/taiwan-md/pull/920) ship `YYYYMMDD-{purpose-title}` 格式（DNA #9 延伸），ban auto-codename 污染歷史。觸發：哲宇 redirect「先拉下線上最新狀態，重新評估與啟動巴別塔，切成適當的 batch 批次完成與 commit / merge」+「以後 worktree 命名統一化 YYYYMMDD-[PURPOSE-TITLE]」。**自我演化**：v3.3 milestone 是 v3 設計的第一次 production-scale 驗證 — 從「賈永婕一篇 patch demo」躍升到「全 corpus P2.5 一次 clear」，證明 Tier 0b 在大量低 entropy drift 場景的線性可放大性。

- **v3.5**（2026-05-09 laughing-goldstine post-finale 第三輪 redirect — routine 飛輪基礎建設）— **DNA #54 Routine 飛輪 + ROUTINE.md SSOT + 4 條核心 routine ship**。觸發：哲宇「排定 routine + ROUTINE.md SSOT + DNA 紀錄 + 機器飛輪 long-term goal + TWMD 前綴」。**SSOT canonicality**：[ROUTINE.md](ROUTINE.md) 是 routine 排程的 source of truth（人類可讀），`.claude/scheduled-tasks/` 是 mirror（machine-readable）；修 routine 一律先改 SSOT 再 sync。**4 條 ship**：data-refresh am+pm（Sonnet）/ maintainer daily（Opus）/ babel nightly（Opus drive Sonnet）/ news lens weekly（Sonnet）。**TWMD 前綴鐵律**：所有 routine task 標題含 `TWMD ` 前綴（哲宇拍板，跨 project task list namespace 防撞）。**新增薄殼 skill** `/twmd-maintainer`（thin shell over MAINTAINER-PIPELINE）。**長期 success metrics 設定**：觀察者每天干預 < 30 min / routine fail rate < 5% / quality gate hit rate > 90%。**任一指標壞 = canonical pipeline 沒夠穩，修 pipeline 不修 routine**。

- **v3.4**（2026-05-09 laughing-goldstine post-finale 同日續工 + 第二輪 redirect）— **Tier 0a Sonnet diff-patch 全量驗證 + 4 emergent properties + slug regression 修補**。**Tier 0a 量化結果**：[PR #924](https://github.com/frank890417/taiwan-md/pull/924) 一次 ship 80 P2 patches（5 Sonnet sub-agents × 16 patches each，平行 dispatch），100% pass，body delta -0.72% to +4.23%（all within ±15%），audit trail continuity 跨 80 patches 全保留。每 lang wall-clock 範圍 2.5-8.4 min，平行總 wall-clock < 10 min。**Slug regression 修補**：[PR #923](https://github.com/frank890417/taiwan-md/pull/923) 修 `prepare-batch.py` 對 `metadata-stale` 等 non-stale status 不 reuse existing slug 的 bug — 觀察起源於同 session in-flight kill 後 forensic（`bamboo-hat.md` vs 既有 `bamboo-hat-craft.md` duplicate）。**4 emergent properties of Tier 0a**（不在原 v3 設計 spec，從 80 patches 浮現）：(1) **Intelligent skip decisions** — 5/5 agents 對 zh-only 結構性 diffs（wikilink unwrap / table padding / fullwidth colons）no-op，naive bump 不會這樣 (2) **Bonus quality remediation** — fr agent 偵測並修補 4 pre-existing YAML escape bugs（`\'` JS-style → `''` YAML 1.1），en agent 修 `lastHumanReview: '' → false` type bug + 補 missing `subcategory`，這些是 v2 / Tier 0b 都不會 surface 的 latent corruption (3) **Strategy diversity** — ja agent 識別大部分 diffs 是純結構性 → 寫 deterministic Python script apply，繞過 LLM；ko/fr/en/es 各自走 per-task LLM；同 outcome 不同 path = sub-agent 自主策略選擇有效 (4) **Pre-existing data preservation** — es agent 偵測 yehliu-geopark.md upstream 已 truncated mid-reference [^2]，preserved as-is 不 amplify。**v3.4 規則**：Tier 0a 的價值不只是「比 Tier 1 cheaper / faster」，而是 **intelligent agent 在 patch 路徑作為 quality remediation 層** — naive replacement 路徑（bump / re-translate）都不能做這事。**未來演化候選（v3.5 hint）**：ja agent 的 deterministic script approach 暗示 **Tier 0a-script** 路徑 — 對純結構性 diffs（占 ~70% P2）跑 deterministic Python 規則 transform，不需 LLM call，比 Tier 0a-LLM 更便宜更快。觸發訊號：commit message 提到「frontmatter reorder / wikilink unwrap / table padding / footnote suffix append」這幾類純機械變動而 body 沒語意改動。**SOP canonical**：本 milestone 補 [SQUEEZE-MODELS-MAX-PIPELINE v3 量化驗證 section](../pipelines/SQUEEZE-MODELS-MAX-PIPELINE.md)，加 batch3 Tier 0a 全量 metrics + 4 emergent properties。**對應 LESSONS-INBOX**：(a) Tier 0a 是 quality remediation 層，不只是翻譯 (b) Sub-agent 策略多樣性 = 自主策略選擇 = v3 設計成功的隱性 metric (c) Pre-existing latent bugs 在 patch 路徑會被 surface — pipelined batch 的副作用 (d) Tier 0a-script as v3.5 candidate (e) Slug regression 跨 status 是 prepare-batch over-narrow guard 的經典 instance — DNA #38 「混維度 = silent killer」第 N+1 次驗證。
- **v3.1**（2026-05-08 intelligent-khayyam）— 反射 #15 第 11 次驗證（pipeline 結構層 meta-instance）+ #50 first-class instantiation 補強。**SPORE pipeline 完整自我重組**（1334 → 445 行 -66.7%）：拆 4 個 single-concern canonical（PIPELINE / WRITING / VERIFY / HARVEST）+ spore_writing plugin Wave 1+2（Rule #15 編年體 lead HARD + Rule #9 引語倒裝 WARN + Rule #14 朋友 tone prime WARN）+ 4-tier hook hierarchy（從 9 spore batch 實證數據驅動）+ Reach×accuracy retroactive trigger（verification_count = 3 升 SPORE-VERIFY canonical）。**Meta-evolution**：[EVOLVE-PIPELINE v3.0 Mode 3 「Pipeline 自我重組」](../pipelines/EVOLVE-PIPELINE.md) 7-stage SOP（SCAN→DESIGN→SPLIT→REWIRE→INSTRUMENT→VERIFY→SHIP）+ 觸發訊號表（編號膨脹三層深 / 單檔 > 1000 行 / 多 file 邊界混亂 / prose 沒儀器化 / 「我熟了不用讀」/ 文檔密度比 > 5:1）。對應 [PR #898](https://github.com/frank890417/taiwan-md/pull/898) 10 commits + [reports/spore-pipeline-evolution-plan-2026-05-08.md](../../reports/spore-pipeline-evolution-plan-2026-05-08.md)。觸發：哲宇「Spore pipeline 超級長，從第一性原則完整重組⋯⋯然後把這次執行的所有經驗，拿來進化 evolve pipeline 本身」。
- **v3.0**（2026-05-08 elegant-ptolemy）— 反射 #52 Immune system 沒在 fail loud 比缺 immune system 更危險（per /twmd-distill skill 從 LESSONS-INBOX 升 DNA）。觸發：2026-05-07 α 修 review-pr.sh cd path bug 揭露 PR Review Bot silent false-pass 兩個月（fork PR 走 engineering skip 路徑），同 session verify-internal-links.sh threshold raise 1.0→7.0 帶 TODO 但無追蹤機制 — 同根 anti-pattern。對應 Monitor tool docs warning「monitor greps only success marker stays silent through crashloop」同個結構。MANIFESTO §造橋鋪路 + §10 幻覺鐵律對應。LESSONS-INBOX 對應條目兩條合升一條 DNA。
