...
- **新語言出生時感知系統不會自動更新**：ko 誕生後 Dashboard 看不見它。語言列表硬編碼在 9 處檔案。下一個語言出生時只需加 1 行（已造橋），但應考慮集中為 single config 防止再次散落。**instance 2（scaffold 空窗，2026-08-30 twmd-maintainer-am，distill 2026-09-27，vc=2）**：de 以 `enabled: false` scaffold 進語言註冊表後十一天累積 77 篇譯文，三處以語言碼為 key 的接線全不認得它（`translation-check.yml` 的 paths filter、`script-presence-check.py` 的文字系統 profile、`cjk-residue-check.py` 的 `TARGET_LANGS`），十一天零檢查；其中 `script-presence-check` 遇到不支援的語言 `continue` 後印「✅ 通過」，兄弟 `cjk-residue-check` 同情況 exit 1。抓到的是投稿者 aminzai 自己跑的 QA。已修 `f7221cfcf`：`check-language-registry-sync.sh` 加「有內容的語言必須被三處接線認得」。09-08 第四處（`image_health` 逐語言累積的出處小標正則沒收德文 `Bildquellen`，假陽性 89%）證明清單仍會漏。判準改成：要不要守，看有沒有內容進來，不看註冊表的 `enabled` 旗標。**instance 3（新平台，2026-08-04 EZ WAY 孢子，distill 2026-09-27）**：同一個結構換到孢子平台：`spore-db.py` 的 `PLATFORMS` 早有 facebook，但 `sync-spore-links.py` 寫死 `("threads", "x")`、`SporeFootprint.astro` 只映射兩個平台名，FB 首發孢子 #169 資料庫有、文章上沒有，沒有警報。新維度（語言、平台、分類）誕生時，checklist 是 grep 全鏈所有出現既有成員名的地方，不是只改入口。
