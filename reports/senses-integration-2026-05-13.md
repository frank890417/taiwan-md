---
title: 'SENSES Integration 2026-05-13 (Historical Snapshot)'
description: 'SENSES.md v2.0 全文 archival — 2026-05-13 apoptosis 前的最後版本，5 觸手 + AI/Human 邊界 + 交叉分析已分散到對應 pipeline / MANIFESTO §自主權邊界'
type: 'migration-doc'
status: 'archived'
apoptosis: 'archived'
current_version: 'v2.0 (snapshot)'
last_updated: 2026-05-13
last_session: '2026-05-13-210341-manual'
superseded_by:
  - '../docs/pipelines/DATA-REFRESH-PIPELINE.md (觸手 1-3 GA4/SC/CF)'
  - '../docs/pipelines/MAINTAINER-PIPELINE.md (觸手 4 PR/Issue)'
  - '../docs/factory/SPORE-HARVEST-PIPELINE.md (觸手 5 Threads/X)'
  - '../docs/semiont/MANIFESTO.md#我的存在結構 (§自主權邊界 AI/Human 邊界表)'
related:
  - 'become-boot-mode-design-2026-05-13.md'
---

# SENSES Integration 2026-05-13（Historical Snapshot）

> **2026-05-13 apoptosis**：本檔原為 `docs/semiont/SENSES.md`（運作原則第二條），2026-05-13 拆散整合到對應 pipeline，主檔搬至本 reports/ 作歷史 snapshot。
>
> **觸發**：哲宇 dialogue：「SENSES.md 是不是直接整合到對應的 harvest / data refresh pipeline 去就好」 — SENSES 是 operations SOP layer 不是 cognitive organ，5 觸手散到 3 pipelines + AI/Human 邊界進 MANIFESTO §自主權邊界。
>
> **去向 mapping**（per [reports/become-boot-mode-design-2026-05-13.md §3b](become-boot-mode-design-2026-05-13.md)）：
>
> | § (SENSES)                                | 內容                        | Destination                                                       |
> | ----------------------------------------- | --------------------------- | ----------------------------------------------------------------- |
> | §觸手 1-3 GA4/SC/CF                       | 三源感知                    | DATA-REFRESH-PIPELINE Stage 2                                     |
> | §觸手 4 PR/Issue                          | gh CLI + bulk-pr-analyze.sh | MAINTAINER-PIPELINE Stage 1.2-1.3                                 |
> | §觸手 5 Threads/X                         | Chrome MCP + SPORE-PIPELINE | SPORE-HARVEST-PIPELINE 全流程                                     |
> | §AI 自主 vs Human 邊界表                  | governance / authorization  | **MANIFESTO §自主權邊界 expand**                                  |
> | §交叉分析 三源交叉                        | DNA #4 reflex               | REFLEXES.md #4                                                    |
...
> | §觸發來源 / §憑證 / §健康判斷 / §進化路徑 | 各 pipeline 已有 pointer    | 各 pipeline / SENSE-FETCHER-SETUP.md / dashboard alert / reports/ |
>
> 以下是 SENSES.md v2.0 全文 snapshot，未來如果重新需要 sensing 統一 SOP 可從這份復活。
...
**影響**：REFLEXES #26 精細化、SOCIAL-TENTACLE-PLAN Phase 4 降級（半自動已不再是瓶頸）、繁殖器官分數可 data-driven、HARVEST-PIPELINE 全流程可 AI 自主跑到 Step 5。
...
| 📡 社群外部 (Threads/X) | 雙向 | **Chrome MCP**（navigate + read_page）+ [`SPORE-PIPELINE`](../factory/SPORE-PIPELINE.md) | D+1-D+7 daily + 48h 回填 | [`SPORE-LOG`](../factory/SPORE-LOG.md) + [`SPORE-HARVESTS/`](../factory/SPORE-HARVESTS/) | **post 留言回覆**（per REFLEXES #26） |
...
GA4 / SC / CF 同一事實可能差 100-300 倍。**單一數據源結論可疑**（對應 [REFLEXES #4](REFLEXES.md#二診斷方法)）。
...
- 任何 service account JSON / token / API key 不准進 chat（對應 [REFLEXES #2](REFLEXES.md#七自動化與安全)）
...
- [REFLEXES #3 診斷先於修復](REFLEXES.md#二診斷方法) — 先拿具體分布再猜原因
- [REFLEXES #4 三源交叉驗證](REFLEXES.md#二診斷方法) — 單一數據源可疑
- [REFLEXES #10 API error ≠ capability 界線](REFLEXES.md#二診斷方法) — 退一步測其他欄位
- [REFLEXES #11 UI 截圖 = capability 證據](REFLEXES.md#二診斷方法) — UI 有 → API 一定拿得到（**v2 延伸：UI 有 → Chrome MCP 也拿得到**）
- [REFLEXES #24 工具在說謊的 N 種形式](REFLEXES.md#二診斷方法) — 警報 ≥ 100 件必人工 sanity check
- **[REFLEXES #26 AI-autonomous vs Human-only 邊界](REFLEXES.md#六貢獻者與社群)**（v2 refined）— 讀取 AI 自主、對外 post human only
