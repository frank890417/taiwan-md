...
JA_SHINJITAI_OVERLAP = frozenset("国学画誉尽")
# 2026-09-26 加「尽」：日文新字體（盡→尽，力を尽くす／尽力／無尽），ja 語料 1,083 篇裡 190 篇有它，
# 委派層一隻 agent 為了讓閘門變綠把「力を尽くし」改寫成別的說法。用語料量過交集而不是憑印象：
# 字集裡其餘字在 ja 語料最多只出現在 12 篇（关／发／为／这⋯），而且都不是日文字形（日文寫
# 関／発／為／這），那些命中是真的簡體殘留，照擋。上面「交集只有四個字」的說法到此修正為五個。
